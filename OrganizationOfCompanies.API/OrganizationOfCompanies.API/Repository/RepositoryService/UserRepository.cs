    ...
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<UserViewModel> GetUserbyId(int userId)
        {
            try
            {
                ...
                _sql.Clear();
                _sql.Append(@$"SELECT email, ""name"", yearsofage, cpf, city, state, address, category, membershipdate FROM public.users where id = {userId};");
        ...
        public async Task<IEnumerable<UserViewModel>> GetAllUser()
        {
            try
            {
                ...
                _sql.Clear();
                _sql.Append(@$"SELECT email, ""name"", yearsofage, cpf, city, state, address, category, membershipdate FROM public.users;");
        ...
        public async Task<bool> PostRegistration(int userId, int eventId)
        {
            try
            {
                ...
                _sql.Clear();
                _sql.Append(@$"INSERT INTO public.userevent (userid, enventid, startdate) values
                                ({userId}, {eventId}, now());");
        ...
        public async Task<int> UserQuantityEvent(int eventId)
        {
            try
            {
                ...
                _sql.Clear();
                _sql.Append(@$"SELECT count(u.userid) FROM userevent u where u.enventid = 1;");
        ...
        public async Task<int> VacanciesQuantityEvent(int eventId)
        {
            try
            {
                ...
                _sql.Clear();
                _sql.Append(@$"select e.vacancies from events e where e.id = 1;");
